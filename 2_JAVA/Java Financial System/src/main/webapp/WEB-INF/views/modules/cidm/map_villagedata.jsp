...
<html>
	...
	<body class="hold-transition skin-blue sidebar-mini">
		...
		<script type="text/javascript" src="${ctxContents }/resource/dist/js/dnJs.js"></script>
		<script>
		    ...
		    function addMarkers(){
			     ...
			     if(markerArr!=null&&markerArr!=undefined){
			     	for (var i = 0; i < markerArr.length; i++) {  
				            ...
				            var p0 = markerArr[i].gaodeCoordinateLat;  
				            var p1 = markerArr[i].gaodeCoordinateLng;  
	 						if(null==p0){
	 							continue;
				            }
							...
							marker.id = markerArr[i].villageId;
				            marker.type = markerArr[i].type;
				            marker.name = markerArr[i].name;
							
							marker.on("click", function(e) {
								var s = e.target;
								if(7==s.type){//小区点击转到楼栋页面
									//window.open(encodeURI(encodeURI("<%=basePath %>/CIDM/map/toBaiduMap/?name="+s.name+"&id="+s.id+"&type=7")),"_self");
