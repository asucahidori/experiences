		...
		<script>
		    ...
		    function addMarker(i, d) {
		        ...
		        if(type=="4"){
					...
					map.setZoomAndCenter(18, [d.location.getLng(), d.location.getLat()]);
					/* var marker = new AMap.Marker({
			            map: map,
			            position: [d.location.getLng(), d.location.getLat()]
			        });
		    ...
		    function addMarkers(){
			     ...
			     if(markerArr!=null&&markerArr!=undefined){
			     	for (var i = 0; i < markerArr.length; i++) {  
							...
							marker.on("click", function(e) {
								var s = e.target;
								if(8==s.type){//小区点击转到楼栋页面
									location.href=encodeURI(encodeURI("${ctxContents }/map/toBuilding?name="+s.name+"&id="+s.id+"&type="+s.type+"&tinyVillageId=0"));
									//window.open(encodeURI(encodeURI("<%=basePath %>/CIDM/map/toBaiduMap/?name="+s.name+"&id="+s.id+"&type=8")),"_self");
								}
