...
if (themeSelector) {
	...
	function selectChange(value) {
		...
		if (theme != 'default') {
			...
			require([ 'http://echarts.baidu.com/doc/example/theme/' + theme ],
					function(tarTheme) {
						curTheme = tarTheme;
 ...
 * lineNumbers: true } ); editor.setOption("theme", 'monokai');
 * 
 * 
...
function needMap() {
	...
	return href.indexOf('Map') != -1 || href.indexOf('mix3') != -1
			|| href.indexOf('mix5') != -1 || href.indexOf('dataRange') != -1;

}

var echarts;
var developMode = false;
function developModeMethod() {
	if (developMode) {
		window.esl = null;
		window.define = null;
		window.require = null;
			...
			function loadedListener() {
				...
				require.config({
					packages : [ {
						...
						name : 'BMap',
						location : '../assets/echarts/extension/BMap/src',
						main : 'main'
					} ]
				});
				launchExample();
...
function launchExample() {
	...
	// 按需加载
	isExampleLaunched = 1;
	require([ 'echarts', 'http://echarts.baidu.com/doc/example/theme/default', // +
	// hash.replace('-en',
	// ''),
