...
/* 2) Between Sales.Orders and Production.Shippers  */
alter table Sales.Orders 
...
alter table Sales.Orders 
	add constraint fk_Orders_Employeesforeign foreign key(EmployeeID)
...
alter table Sales.[Order Details]
	add constraint df_Discount_OrderDetails default 0 for UnitPrice;
go
...
alter table Production.Products
	add constraint fk_Products_Categories foreign key(CategorieID)
		references Production.Categories(CategorieID);
go
...
alter table Production.Products
	add constraint ck_ReorderLevel_Products check(ReorderLevel
	>= 0);
go
...
alter table Employees.EmployeeID
	add constraint fk_Employees_Employees foreign key(ReportsTo)
		references Employees.EmployeeID(EmployeeID);
go

alter table Employees.BirthDate
...
alter table HumanResources.Territories
	add constraint fk_Territories_Region foreign key(TerritorieID)
		references HumanResources.Region(TerritorieID);
go
...
alter table HumanResources.EmployeeTerritories
	add constraint fk_EmployeeTerritories_Territories foreign key(TerritorieID)
		references HumanResources.Territories(TerritorieID);
go
